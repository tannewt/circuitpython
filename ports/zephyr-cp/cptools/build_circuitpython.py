...
async def build_circuitpython():
    ...
    autogen_modules_fn = mpconfigboard_fn.parent / "autogen_modules.toml"
    ...
    hal_source = []
    autogen_modules = tomlkit.document()
    autogen_modules.add(
        tomlkit.comment("This file is autogenerated when a board is built. Do not edit.")
    )
    for module in sorted(list(top.glob("shared-bindings/*")) + list(portdir.glob("bindings/*"))):
        if not module.is_dir():
            continue
        enabled = module.name in enabled_modules
        # print(f"Module {module.name} enabled: {enabled}")
        autogen_modules.add(module.name, enabled)
        circuitpython_flags.append(f"-DCIRCUITPY_{module.name.upper()}={1 if enabled else 0}")

        if enabled:
            hal_source.extend(top.glob(f"bindings/{module.name}/*.c"))
